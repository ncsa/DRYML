...
from dryml.context.new.context_tracker import contexts,  \
...
from dryml.core2.util import pickler, unpickler, get_unique_objects
...
class process_executor(object):
    def __init__(
        ...
        self.args_def_ser = pickler(args_def)
        self.kwargs_def_ser = pickler(kwargs_def)
    ...
    def __call__(self, ctx_ret_q):
        ...
        # Undill args/kwargs
        args_def = unpickler(self.args_def_ser)
        kwargs_def = unpickler(self.kwargs_def_ser)

        # Load repo
        import os
        ic(os.listdir('/tmp'))
        repo = Repo(self.ctx_repo)
        ic(os.listdir(repo.dir))

        ic(args_def, kwargs_def)

        args = repo.load_object(args_def)
        kwargs = repo.load_object(kwargs_def)
    ...
    def _func_dec(f):
        ...
        def wrapped_func(
            ...
            if ctx_reqs is None:
                ...
                ctx_reqs = get_context_requirements((
                    args, kwargs))
            ...
            if use_existing_context:
                ...
                # Save objects to repo
                with manage_repo(repo=repo) as process_repo:
                    # Save all objects to the repo
                    ic("saving objects", args, kwargs)
                    process_repo.save_object((args, kwargs))

                    # Concretize args/kwargs to send
                    send_args = concretize_definition(args)
                    send_kwargs = concretize_definition(kwargs)

                    # create the process
                    executor = process_executor(
                        f=f, ctx_reqs=ctx_reqs,
                        ctx_repo=process_repo.dir,
                        #update_obj_defs=update_obj_defs,
                        verbose=verbose,
                        args_def=send_args,
                        kwargs_def=send_kwargs)
                    ...
                    num_to_fetch = 1 # The return value
