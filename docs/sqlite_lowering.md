...
- active read-view relation operations for domain filtering, parent traversal, child traversal, child-exists semijoins, intersection, union, temp materialization, owner projection, keyset paging, count estimates, and exact-safe counts;
...
- owner projection through the existing recursive SQLite owner relation, with owner count diagnostics;
