...
Exact stored-root `ConcreteDefinition` count is exact-safe after backend equality confirmation and may bypass final query verification. Stable-hash candidate anchors for general selectors are not exact-safe; Python verification remains authoritative for those paths.
