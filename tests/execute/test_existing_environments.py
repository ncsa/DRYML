...
def _interpreter() -> str:
    """Return the worker interpreter without relying on coordinator state."""
    import sys

    return sys.executable


def _runtime_details() -> tuple[str, int, str | None, str]:
    """Return worker runtime identity without relying on coordinator imports."""
    from importlib.metadata import version
    import os
    import sys

    return sys.executable, os.getpid(), os.environ.get("PYTHONPATH"), version("dryml")
...
def test_existing_supplied_interpreters_execute_real_workloads(tmp_path: Path, variable: str):
    ...
    if variable == "DRYML_TEST_CONDA_PREFIX":
        target = target / ("python.exe" if os.name == "nt" else "bin/python")
        assert target.is_file(), "DRYML_TEST_CONDA_PREFIX must contain an existing Python"
    executor = Executor(SubProcessConfig(spool_directory=tmp_path, python_executable=target))
    try:
        assert Path(executor.run(lambda: __import__("sys").executable)).resolve() == target.resolve()
    finally:
...
def test_existing_conda_run_selector_uses_wrapper_and_installed_runtime(tmp_path: Path):
    ...
    try:
        ...
        worker_executable, worker_pid, pythonpath, dryml_version = future.result(timeout=30)
        assert Path(worker_executable).resolve() == (prefix / ("python.exe" if os.name == "nt" else "bin/python")).resolve()
